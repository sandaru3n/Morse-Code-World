...
import { getMp3CutterPageCopy, getMp3CutterPageLinks } from "@/lib/i18n/toolPages/mp3Cutter";
import { SITE_NAME, absoluteUrl } from "@/lib/site";
import { mp3CutterPath } from "@/lib/i18n/routes";
import { createWebAppSchema, createWebPageSchema } from "@/lib/seo/schemas";

export function Mp3CutterPageView({ locale }: { locale: HomeLocale }) {
  const copy = getMp3CutterPageCopy(locale);
  const links = getMp3CutterPageLinks(locale);
  const pageUrl = absoluteUrl(mp3CutterPath(locale));
  ...
  const webPageSchema = createWebPageSchema({
    ...
    url: pageUrl,
    description: copy.description,
    speakableSelectors: ["#site-summary-mp3-cutter"]
  });
      ...
      <div className="flex flex-1 pt-[4.5rem]">
        <main className="flex-1 overflow-y-auto p-3 sm:p-5 lg:p-8">
          <div className="mx-auto max-w-3xl">
            ...
            <section
              id="site-summary-mp3-cutter"
              data-ai-summary="true"
              className="mt-4 rounded-2xl border border-sky-200/80 bg-sky-50/90 px-4 py-4 dark:border-sky-900/40 dark:bg-sky-950/25"
              aria-labelledby="site-summary-heading-mp3-cutter"
            >
              <h2 id="site-summary-heading-mp3-cutter" className="font-headline text-base font-bold sm:text-lg">
                {copy.summaryTitle}
              </h2>
            ...
            <section
              ...
              aria-labelledby="mp3-cutter-about"
            >
              <h2 id="mp3-cutter-about" className="font-headline text-lg font-bold sm:text-xl">
                {copy.aboutHeading}
              </h2>
