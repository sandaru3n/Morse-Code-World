...
import { getPicturePageCopy, getPicturePageLinks } from "@/lib/i18n/toolPages/picture";
import { SITE_NAME, absoluteUrl } from "@/lib/site";
import { pictureTranslatorPath } from "@/lib/i18n/routes";
import { createWebAppSchema, createWebPageSchema } from "@/lib/seo/schemas";

export function PictureMorseTranslatorPageView({ locale }: { locale: HomeLocale }) {
  const copy = getPicturePageCopy(locale);
  const links = getPicturePageLinks(locale);
  const pageUrl = absoluteUrl(pictureTranslatorPath(locale));
  ...
  const webPageSchema = createWebPageSchema({
    ...
    url: pageUrl,
    description: copy.description,
    speakableSelectors: ["#site-summary-picture", "#picture-morse-about"]
  });
      ...
      <div className="flex flex-1 pt-[4.5rem]">
        <main className="flex-1 overflow-y-auto p-3 sm:p-5 lg:p-8">
          <div className="mx-auto max-w-3xl">
            ...
            <section
              id="site-summary-picture"
              data-ai-summary="true"
              className="mt-4 rounded-2xl border border-sky-200/80 bg-sky-50/90 px-4 py-4 dark:border-sky-900/40 dark:bg-sky-950/25"
              aria-labelledby="site-summary-heading-picture"
            >
              <h2 id="site-summary-heading-picture" className="font-headline text-base font-bold sm:text-lg">
                {copy.summaryTitle}
              </h2>
            ...
            >
              <h2 id="picture-morse-about" className="font-headline text-lg font-bold sm:text-xl">
                {copy.aboutHeading}
              </h2>
              <div className="mt-4 space-y-4 text-sm leading-relaxed text-slate-600 dark:text-slate-400 sm:text-base">
                ...
                <p>
                  ...
                  ,{" "}
                  <Link href={links.audio} className="font-semibold text-emerald-600 underline underline-offset-2 dark:text-primary-container">
                    {copy.linkAudio}
                  </Link>
                  .
                </p>
            ...
            <section className="mt-8 rounded-2xl border border-slate-200/80 bg-white/80 p-5 dark:border-outline-variant/25 dark:bg-surface-container/40 sm:p-6">
              <h2 className="font-headline text-lg font-bold sm:text-xl">{copy.qualityHeading}</h2>
              <div className="mt-4 space-y-4 text-sm leading-relaxed text-slate-600 dark:text-slate-400 sm:text-base">
                <p>{copy.qualityP1}</p>
                <p>{copy.qualityP2}</p>
              </div>
